...
async function downloadImage(imageUrl) {
    ...
    const imagePath = path.join(TEMP_DIR, `ad-image-bot1-${Date.now()}${ext}`);
...
async function processOnePostBot1(initialPostData) {
    ...
    try {
        ...
        while (true) {
            ...
            // 💡 --- فحص التكرار (الحل النهائي: إذا نُشرت سابقاً يُحذف عمود القروب بالكامل ويتم التخطي) ---
            ...
            if (logData && logData.length > 0) {
                await logToDashboard(`🛡️ [حماية] الإعلان (#${initialPostData.id}) نُشر مسبقاً في المجموعة (${targetGroup.name}) بواسطة ${BOT_ID}! جاري تصفير الحقل والتخطي فوراً...`, 'warn');
                
                // 🧹 تصفير عمود القروب الخاص بالبوت الأول لكي لا يعلق مجدداً
                await supabase.from('publish_queue').update({ 
                    bot1_group: null, 
                    ai_final_text1: null 
                }).eq('id', initialPostData.id);
            ...
            try {
                ...
                // 🧹 تصفير عمود القروب الخاص بالبوت الأول فوراً بعد النشر الناجح
                await supabase.from('publish_queue').update({
                    bot1_group: null,
                    ai_final_text1: null,
                ...
                // تصفير عمود القروب حتى عند حدوث خطأ لكي لا يعلق الإعلان
                await supabase.from('publish_queue').update({ 
                    bot1_group: null,
                    ai_final_text1: null,
        ...
        await logToDashboard(`❌ خطأ عام في البوت الأول: ${err.message}`, 'error');
        ...
        await logToDashboard(`🧹 اكتملت العملية وأُغلق متصفح البوت الأول بأمان.`, 'info');
    }
}
...
module.exports = processOnePostBot1;
